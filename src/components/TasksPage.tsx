  ...
  return (
    <div className="animate-fade">
      ...
      {isManager && activeTab === 'team' ? (
                ...
                return (
                  <div key={p.user_id} className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="w-full h-2 bg-dark-600 overflow-hidden">
        ...
        <>
          {total > 0 && (
            <div className="w-full h-3 bg-dark-700 overflow-hidden mb-6">
              <div className="h-full bg-gradient-to-r from-emerald-500 to-emerald-400 progress-bar" style={{ width: `${total > 0 ? (done / total) * 100 : 0}%` }} />
            </div>
          )}
          ...
          {filtered.length === 0 ? (
            ...
            <div className="space-y-3">
              {filtered.map((t) => {
                ...
                return (
                  <div key={t.id} className={`relative flex items-center gap-4 p-5 transition-all duration-300 ${
                    ...
                    <div className={`w-8 h-8 border-2 flex items-center justify-center transition-all cursor-pointer ${
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {(() => {
                          ...
                          return <span className={`text-[10px] px-2 py-0.5 ${priorityBadge.color}`}>{priorityBadge.icon} {priorityBadge.label}</span>
                        })()}
                        ...
                        {t.category && <span className="text-[10px] px-2 py-0.5 bg-purple-500/20 text-purple-300">{t.category}</span>}
                      </div>
                    ...
                    <div className="text-right flex flex-col gap-1">
                      ...
                      {!isDone && t.status === 'pending' && (
                        <button onClick={() => handleStart(t.id)} className="text-xs text-blue-400 hover:bg-blue-500/10 px-2 py-1">開始</button>
                      )}
                      {!isDone && t.status === 'in_progress' && (
                        <button onClick={() => handleComplete(t)} className="text-xs text-emerald-400 hover:bg-emerald-500/10 px-2 py-1">完成</button>
                      )}
                    </div>
                  </div>
                )
